...
# ========================================================================================================
    @classmethod
    def displayMSJ(cls,id):
        query = "SELECT users.id, users.first_name as receiver, users2.first_name as sender_name, users2.last_name as sender_lastname,  messages.message_info, messages.sender_id, messages.message_id FROM messages LEFT JOIN users ON users.id = messages.id LEFT JOIN users as users2 ON users2.id = messages.sender_id WHERE users.id = %(id)s;"

        results = connectToMySQL('loginform').query_db(query,id)
